...
class PullRequestHandler extends RepoWebhookHandler {
    public function handle() {
        ...
        if(!($manifest["pulls"] ?? true)) throw new StopWebhookExecutionException("Poggit Build not enabled for PRs");
        if(isset($manifest["branches"]) and !in_array($pr->base->ref, (array) $manifest["branches"])) {
            throw new StopWebhookExecutionException("Poggit Build not enabled for branch");
        }
        ...
        $commits = Poggit::ghApiGet("repos/{$repo->full_name}/pulls/{$pr->number}/commits", $token);
        $commitMessages = [];
        foreach($commits as $commit) {
            $commitMessages[] = $commit->commit->message;
        }
        $changedFiles = [];
        $files = Poggit::ghApiGet("repos/{$repo->full_name}/pulls/{$pr->number}/files", $token);
        ...
        ProjectBuilder::buildProjects($zipball, $repo, $projects, $commitMessages, $changedFiles, $cause, function (WebhookProjectModel $project) : int {
