...
class VirionListModule extends Module {
    ...
    public function output() {
        ...
        $libs = Mysql::query("SELECT
                ...
                UNIX_TIMESTAMP(b.created) lastBuildDate,
                b.main antigen,
                IF(LENGTH(vb.version) > 0, CONCAT('v', vb.version), 'Unknown version') lastVersion
            FROM projects INNER JOIN repos ON projects.repoId = repos.repoId
            LEFT JOIN (SELECT
                    virion_project.projectId virionProjectId,
                    MAX(virion_build.buildId) lastVirionBuild,
            ...
            LEFT JOIN builds b ON t.lastVirionBuild = b.buildId
            LEFT JOIN virion_builds vb ON t.lastVirionBuild = vb.buildId
    ...
    private function displayLib(\stdClass $lib) {
        ...
        <li>
            ...
            <p class="remark">Last updated: <span class="time" data-timestamp="<?= $lib->lastBuildDate ?>"></span> (<?= $lib->lastVersion ?>)</p>
        </li>
        <?php
    }
}
