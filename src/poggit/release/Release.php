...
class Release {
    ...
    public static function getReviewQueue(int $maxState, int $count = 30, int $minState = 0, string $minAPI = null): array {
        ...
        $plugins = Mysql::query("SELECT
            ...
            ORDER BY r.assignee = ? DESC, flags & ?, flags & ?, r.assignee IS NOT NULL, state DESC, updateTime DESC LIMIT $count",
            "iiiii", Session::getInstance()->getUid(), $minState, $maxState, self::FLAG_OBSOLETE, self::FLAG_OUTDATED);
