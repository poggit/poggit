...
if(php_sapi_name() !== "cli") {
    echo("library_stub.php should only be run from CLI, not web servers!");
    exit(1);
}
if(class_exists("pocketmine\\Server", false)) {
    echo("library_stub.php should only be run from CLI, not PocketMine servers!");
    exit(1);
}
if(substr(__FILE__, -5) !== ".phar") {
    echo "[!] Fatal: library_stub.php should not be executed directly. Run it when it is in a phar file.";
    exit(1);
}
