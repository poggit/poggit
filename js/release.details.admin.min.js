...
k=$("<textarea cols='80' rows='5'></textarea>").appendTo(a),l=$("<textarea cols='80' rows='2'></textarea>").appendTo(a),e=$("<select></select>").append($("<option value='keep' selected>Unchanged</option>")).append($("<option value='rejected'>Rejected</option>")).append($("<option value='draft'>Draft</option>"));a.append($("<div>Change state to: </div>").append(e));var p,q=$("<select></select>").appendTo(a);$("<button>Add</button>").click(function(){var b=k.val(),a=p[q.val()];b+="\n\n"+a.id+" &mdash; "+
...
ghApi(releaseDetails.rejectPath,{body:a},"POST",function(){switch(e.val()){case "rejected":g(PoggitConsts.ReleaseState.rejected,a);break;case "draft":g(PoggitConsts.ReleaseState.draft,a)}})}}});e.change(function(){var a=e.val();"rejected"===a?(h.val("Dear @"+releaseDetails.project.repo.owner+',\n> I regret to inform you that your plugin "**'+releaseDetails.name+'**" (v'+releaseDetails.version+" submitted on "+(new Date(1E3*releaseDetails.created)).toISOString()+") has been rejected."),l.val("Please resolve these issues and submit the plugin again.")):
