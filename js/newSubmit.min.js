var $jscomp=$jscomp||{};$jscomp.scope={};$jscomp.findInternal=function(g,n,k){g instanceof String&&(g=String(g));for(var t=g.length,q=0;q<t;q++){var y=g[q];if(n.call(k,y,q,g))return{i:q,v:y}}return{i:-1,v:void 0}};$jscomp.ASSUME_ES5=!1;$jscomp.ASSUME_NO_NATIVE_MAP=!1;$jscomp.ASSUME_NO_NATIVE_SET=!1;$jscomp.defineProperty=$jscomp.ASSUME_ES5||"function"==typeof Object.defineProperties?Object.defineProperty:function(g,n,k){g!=Array.prototype&&g!=Object.prototype&&(g[n]=k.value)};
$jscomp.getGlobal=function(g){return"undefined"!=typeof window&&window===g?g:"undefined"!=typeof global&&null!=global?global:g};$jscomp.global=$jscomp.getGlobal(this);$jscomp.polyfill=function(g,n,k,t){if(n){k=$jscomp.global;g=g.split(".");for(t=0;t<g.length-1;t++){var q=g[t];q in k||(k[q]={});k=k[q]}g=g[g.length-1];t=k[g];n=n(t);n!=t&&null!=n&&$jscomp.defineProperty(k,g,{configurable:!0,writable:!0,value:n})}};
$jscomp.polyfill("Array.prototype.find",function(g){return g?g:function(g,k){return $jscomp.findInternal(this,g,k).v}},"es6","es3");$jscomp.polyfill("Object.is",function(g){return g?g:function(g,k){return g===k?0!==g||1/g===1/k:g!==g&&k!==k}},"es6","es3");$jscomp.polyfill("Array.prototype.includes",function(g){return g?g:function(g,k){var n=this;n instanceof String&&(n=String(n));var q=n.length;for(k=k||0;k<q;k++)if(n[k]==g||Object.is(n[k],g))return!0;return!1}},"es7","es3");
$jscomp.checkStringArgs=function(g,n,k){if(null==g)throw new TypeError("The 'this' value for String.prototype."+k+" must not be null or undefined");if(n instanceof RegExp)throw new TypeError("First argument to String.prototype."+k+" must not be a regular expression");return g+""};$jscomp.polyfill("String.prototype.includes",function(g){return g?g:function(g,k){return-1!==$jscomp.checkStringArgs(this,g,"includes").indexOf(g,k||0)}},"es6","es3");
...
"Submit Plugin from Draft",e="Save Edit"):2===f.refRelease.state?(c="Save Edit",e="Restore to Draft"):c="Save Edit";null!==c&&a.append($("<span class='action-red'></span>").text(c).click(function(){n("submit")}));null!==e&&a.append($("<span class='action-red'></span>").text(e).click(function(){n("draft")}));a.appendTo(b)}function n(b){var a=!1;$(".submit-deps-version").each(function(){"undefined"===typeof $(this).attr("data-relid")&&(alert("You haven't selected the version in one of the dependencies yet!"),
...
"200px")).append($("<th>Version</th>")).appendTo(b)},function(b){var a=$("<input class='submit-deps-name' size='15'/>"),c=$("<span class='submit-deps-version action'>Choose a version...</span>");a.change(function(){c.text("Choose a version").removeAttr("data-relid")});c.click(function(){B=c;var b=w=Math.random(),d=a.val();p.dialog.dialog("option","title",'Choose a version of "'+a.val()+'"');p.loading.css("display","block");p.error.css("display","none");p.inner.css("display","none");p.dialog.dialog("open");
ajax("submit.deps.getversions",{data:{name:d,owner:"false"},success:function(a){if(b===w)if(0===Object.sizeof(a))p.loading.css("display","none"),p.error.text("There aren't any submitted (and not rejected) plugins on Poggit called "+d).css("display","block");else{p.loading.css("display","none");C=a;p.table.find("tr.submit-deps-version-select-row").remove();p.table.find("tr.submit-deps-version-header").remove();for(var c in a)if(a.hasOwnProperty(c)){var e=$("<span></span>").attr("data-timestamp",a[c].submitTime);
timeTextFunc.call(e.get()[0]);p.table.prepend($("<tr></tr>").addClass("submit-deps-version-select-row").append($("<td></td>").append($("<input type='radio'/>").addClass("submit-dep-dialog-version-radio").attr("name",w).attr("data-value",c).prop("checked",!0)).append($("<label></label>").text(a[c].version))).append($("<td></td>").append($("<input type='checkbox' disabled/>").prop("checked",a[c].preRelease))).append($("<td></td>").append(a[c].stateName)).append($("<td></td>").append(e)))}p.table.prepend("<tr class='submit-deps-version-header'><th>Version</th><th>Pre-release</th><th>Status</th><th>Submitted</th></tr>");
p.inner.css("display","block")}}})});b.append($("<select class='submit-deps-required'></select>").append("<option value='required'>Required</option>").append("<option value='optional'>Optional</option>").wrap("<td></td>").parent()).append(a.wrap("<td></td>").parent()).append(c.wrap("<td></td>").parent())},function(b){var a=b.find("span.submit-deps-version");return"undefined"===typeof a.attr("data-relid")?null:{name:b.find("input.submit-deps-name").val(),version:a.attr("data-version"),depRelId:Number(a.attr("data-relid")),
...
a.change(function(){c.text("Choose a version").removeAttr("data-relid")});c.click(function(){a.css("border","");if(3>a.val().length)a.css("border","1px solid red");else{var b=w=Math.random();B=c;var d=a.val();p.dialog.dialog("option","title",'Choose a version of "'+a.val()+'"');p.loading.css("display","block");p.error.css("display","none");p.inner.css("display","none");p.dialog.dialog("open");ajax("submit.deps.getversions",{data:{name:d,owner:"true"},success:function(a){if(b===w)if(0===Object.sizeof(a))p.loading.css("display",
"none"),p.error.text("You do not own any submitted (and not rejected) plugins on Poggit called "+d).css("display","block");else{p.loading.css("display","none");C=a;var c=!0;p.table.find("tr.submit-deps-version-select-row").remove();for(var e in a)if(a.hasOwnProperty(e)){var f=$("<span></span>").attr("data-timestamp",a[e].submitTime);timeTextFunc.call(f.get()[0]);p.table.append($("<tr></tr>").addClass("submit-deps-version-select-row").append($("<td></td>").append($("<input type='radio'/>").addClass("submit-dep-dialog-version-radio").attr("name",
w).attr("data-value",e).prop("checked",c)).append($("<label></label>").text(a[e].version))).append($("<td></td>").append($("<input type='checkbox' disabled/>").prop("checked",a[e].preRelease))).append($("<td></td>").append(a[e].stateName)).append($("<td></td>").append(f)));c=!1}p.inner.css("display","block")}}})}});$("<div></div>").css("display","flex").append(a).append(c).appendTo(b)},getter:function(){var b=this.$getRow().find("#submit-assoc-parent-version");return"undefined"===typeof b.attr("data-relid")?
null:{releaseId:Number(b.attr("data-relid")),name:this.$getRow().find("submit-assoc-parent-name").val(),version:b.attr("data-version")}},setter:function(b){null!==b&&null!==b.version&&($("#submit-assoc-parent-version").text("Change: v"+b.version).attr("data-version",b.version).attr("data-relid",b.releaseId),$("#submit-assoc-parent-name").val(b.name))}}}var h=[],r=PoggitConsts.Config,F,E,f;ajax("submit.form",{data:{query:window.location.pathname.substr(getRelativeRootPath().length)},success:function(b){if("error/redirect"===
b.action)window.location.replace(b.target),null!==b.message&&alert(b.message);else{var a=$("#body");if("error/bad_query"===b.action)a.html("<h1>400 Bad Request</h1>"),b.text?$("<p></p>").text(b.message).appendTo(a):a.append("<p>"+b.message+"</p>");else if("error/access_denied"===b.action)a.html("<h1>401 Access Denied</h1>"),b.text?$("<p></p>").text("Details: "+b.message).appendTo(a):a.append("<p>Details:&nbsp;"+b.message+"</p>");else if("error/not_found"===b.action)a.html("<h1>404 Not Found</h1>"),
b.text?$("<p></p>").text(b.message).appendTo(a):a.append("<p>"+b.message+"</p>");else if("success"!==b.action)alert("The server returns an unknown result! Please copy (or screenshot) the following data:\n\n"+JSON.stringify(b));else{f=b.submitData;a=b.submitData.last;document.title=b.title;document.getElementById("submit-title-action").innerHTML=b.actionTitle;generateGhLink(b.treeLink).appendTo(document.getElementById("submit-title-gh"));b=f.args.slice(0,3).join("/");$("<a class='colorless-link' target='_blank'></a>").attr("href",
getRelativeRootPath()+"ci/"+b).append($("<img/>").attr("src",getRelativeRootPath()+"ci.badge/"+b+"?build="+f.args[3])).appendTo(document.getElementById("submit-title-badge"));if(null!=a){var c=document.getElementById("submit-intro-last-name");$("<h5></h5>").text("Updates v"+a.version).append($("<sub></sub>").append($("<a class='colorless-link' target='_blank'></a>").attr("href",getRelativeRootPath()+"ci/"+b+"/"+a.internal).text("Dev Build #"+a.internal+" (&"+a.buildId+")"))).appendTo(c);c.style.display=
"block"}g()}}},error:function(b){b=b.getResponseHeader("X-Poggit-Request-ID");window.location.replace(getRelativeRootPath()+"500ise.template?id="+b)}});l.isNextFirstEntry=!0;l.prototype.$getRow=function(){return $(document.getElementById(this.id))};l.prototype.appendTo=function(b){var a=$("<div class='form-row'></div>").attr("id",this.id);$("<div class='form-key'></div>").text(this.name).appendTo(a);var c=$("<span></span>").addClass("explain").html(this.rem),e=$("<div class='form-value'></div>");
this.type.afterRemarks?(c.addClass("explain-before-input").appendTo(e),this.type.appender.call(this,e)):(this.type.appender.call(this,e),c.addClass("explain-after-input").appendTo(e));e.appendTo(a);$("<div class='form-input-react'></div>").appendTo(e);var d=this;if(!this.locked){c=$("<div class='form-value-defaults'></div>");var g=e=null;null!==this.refDefault&&(e=$("<span class='action form-value-import'></span>").text("edit"===f.mode?"Reset":this.isThisFirstEntry?"Copy from last release":"Copy"),
e.click(function(){d.type.setter.call(d,d.refDefault)}));null!==this.srcDefault&&(g=$("<span class='action form-value-import'></span>").text(this.isThisFirstEntry?"Detect from code":"Detect"),g.click(function(){d.type.setter.call(d,d.srcDefault)}));this.prefSrc?(null!==g&&g.appendTo(c),null!==e&&e.appendTo(c)):(null!==e&&e.appendTo(c),null!==g&&g.appendTo(c));c.appendTo(a)}a.appendTo(b)};l.prototype.setDefaults=function(){if(this.prefSrc)if(null!==this.srcDefault)var b=this.srcDefault;else null!==
...
c.addClass(a);else if("object"===typeof a&&a.constructor===Array)for(b=0;b<a.length;++b)this.classes.push(a[b]),c.addClass(a[b])};l.prototype.getValue=function(){return this.locked?null:this.type.getter.call(this)};l.prototype.isValid=function(){return this.validator.call(this)};var B,w,C,p=function(){var b=$("<div id='depSelectDialog'></div>"),a=$("<div></div>").css("font-size","x-large").text("Loading versions..."),c=$("<div></div>").addClass("fallback-error"),e=$("<div></div>");e.append("<p>Select the <strong>recommended</strong> version of the dependency plugin, usually the <em>latest</em> version. You don't need to select older versions; if you want to, mention it in the Description.</p>");
e.append("<p>Keep in mind that the state of your plugin will never be higher than the state of your dependencies (except Featured). For example, if your plugin depends on a release that has not been approved yet, your plugin won't get approved either before the dependency release gets approved.</p>");var d=$("<table></table>");d.appendTo(e);b.append(a).append(c).append(e);b.dialog({autoOpen:!1,width:Math.min(.8*window.innerWidth,600),modal:!0,height:.8*window.innerHeight,position:modalPosition,buttons:{Select:function(){var a=
$("input.submit-dep-dialog-version-radio:checked").attr("data-value"),b=C[a].version;B.text("Change: v"+b).attr("data-relid",a).attr("data-version",b);$(this).dialog("close");C=w=B=void 0}},open:function(a,b){$(".ui-widget-overlay").bind("click",function(){$("#depSelectDialog").dialog("close")})}});return{dialog:b,loading:a,error:c,inner:e,table:d}}()});
