...
200>this.getValue().custom.length?{valid:!1,message:"The custom license must be a open source license, and the full text is required."}:null===this.getValue().type?{valid:!1,message:"A license is required before it can be submitted."}:{valid:!0}}));g.push(new k("perms",f.fields.perms,"Permissions","submit2-perms",G(f.consts.perms,!0),function(){return{valid:!0}}));g.push(D=new k("authors",f.fields.authors,"Producers","submit2-authors",O,function(){return{valid:this.getValue().length>=D.type.minRows,
...
var q=$("<optgroup label='Special'></optgroup>").append($("<option></option>").text("Custom License").attr("value","custom")),f=$("<optgroup></optgroup>").attr("label","Deprecated License IDs");e.change(function(){d.css("display","custom"===e.val()?"block":"none");var a="none"===e.val()||"custom"===e.val();g.hasClass("disabled")!==a&&(a?g.addClass("disabled"):g.removeClass("disabled"))}).appendTo(c);var d=$("<textarea></textarea>").addClass("submit-license-custom").css("display","none").appendTo(c);
b();var g=$("<span></span>").addClass("action disabled").attr("id","licenseView").text("View license details").click(function(){var b=e.val();void 0!==a[b]&&window.open(a[b].reference,"_blank").focus()}).appendTo(c);ajax("licenses.ajax",{data:{},success:function(b){b=JSON.parse(b);b=b.licenses;b.sort(function(a,b){return a.name.localeCompare(b.name)});for(var c=0;c<b.length;c++)if(!1!==b[c].isOsiApproved){var d=$("<option></option>");d.attr("value",b[c].licenseId);d.attr("data-url",b[c].detailsUrl);
d.text(b[c].name+" ("+b[c].licenseId+")");a[b[c].licenseId]=b[c];"undefined"!==typeof l.wannaSet&&b[c].licenseId===l.wannaSet.type&&(d.prop("selected",!0),e.val(b[c].licenseId),g.removeClass("disabled"));d.appendTo(b[c].isDeprecatedLicenseId?f:e)}f.appendTo(e);q.appendTo(e);l.ready=!0}})},getter:function(){if(!this.ready)return this.wannaSet;var a=this.$getRow(),b=a.find(".submit-license-type").val();return{type:b,custom:"custom"===b?a.find(".submit-license-custom").val():null}},setter:function(a){if(this.ready||
"custom"===a.type||"none"===a.type){var b=this.$getRow();b.find(".submit-license-type").val(a.type);"custom"===a.type&&b.find(".submit-license-custom").val(a.custom)}else this.wannaSet=a},afterRemarks:!1}}function H(a,b,c){return{appender:function(l){var e=this,f=$("<select></select>");f.addClass("submit-selinput");this.locked&&f.prop("disabled",!0);for(var d in a)if(a.hasOwnProperty(d)){var g=$("<option></option>");g.attr("value",d);g.text(a[d]);g.appendTo(f)}h(f,b);"function"===typeof c&&f.change(function(a){a=
c.call(e,f.val(),f,a);"string"===typeof a&&f.val(a)});f.appendTo(l)},getter:function(){return this.$getRow().find(".submit-selinput").val()},setter:function(a){this.$getRow().find(".submit-selinput").val(a);"function"===typeof c&&c.call(this,a)},noDefaults:function(){"function"===typeof c&&c.call(this,this.$getRow().find(".submit-selinput").val())},afterRemarks:!1}}function I(a,b,c,f){return{appender:function(e){var d=$("<div></div>");d.addClass("submit-cbgrp-compact");for(var l in a)if(a.hasOwnProperty(l)){var g=
$("<div class='cbinput'></div>"),k=$("<input type='checkbox'/>");k.addClass("submit-cb");k.attr("value",l);k.appendTo(g);g.append(a[l]);g.appendTo(d);this.locked&&k.prop("disabled",!0);h(k,b);h(g,c)}h(d,f);d.appendTo(e)},getter:function(){return this.$getRow().find(".submit-cb:checked").map(function(){return this.value}).get()},setter:function(a){this.$getRow().find(".submit-cb").not(".submit-cb-disabled").each(function(){this.checked=a.map(String).includes(this.value)})},afterRemarks:!1}}function G(a,
b){return{afterRemarks:b,appender:function(b){var c=$("<div></div>");c.addClass("submit-expcb-wrapper");for(var e in a)if(a.hasOwnProperty(e)){var f=$("<div></div>").addClass("submit-expcb-row"),d=$("<div></div>").addClass("cbinput"),g=$("<input type='checkbox'/>").addClass("submit-cb").attr("value",e);g.appendTo(d);this.locked&&g.prop("disabled",!0);d.append("<div class='cblabel'>"+a[e].name+"</div>");d.appendTo(f);$("<div></div>").addClass("remark").text(a[e].description).appendTo(f);f.appendTo(c)}c.appendTo(b)},
getter:function(){return this.$getRow().find(".submit-cb:checked").map(function(){return this.value}).get()},setter:function(a){this.$getRow().find(".submit-cb").not(".submit-cb-disabled").each(function(){this.checked=a.map(String).includes(this.value)})}}}function z(a,b,c,f,e){function d(a,c){var f=$("<tr class='submit-tableentry-row'></tr>");b(f,c);c.locked||(c=$("<td></td>"),$("<span class='action-red'>&cross;</span>").click(function(){a.find(".submit-tableentry-row").length<=e?alert("There must be at least "+
e+" rows!"):f.remove()}).appendTo(c),c.appendTo(f));f.appendTo(a);return f}"undefined"===typeof e&&(e=0);return{appender:function(b){var c=$("<table class='submit-tableentry-table'></table>");a(c);for(var f=0;f<e;++f)d(c,this);c.appendTo(b);this.locked||$("<span class='action'>&plus;</span>").click(function(){d(c,this)}).appendTo(b)},getter:function(){var a=[];this.$getRow().find(".submit-tableentry-row").each(function(){var b=c($(this));null!==b&&a.push(b)});return a},setter:function(a){console.assert(a.constructor===
Array,"values passed for "+this.name+" is not array but ",a);var b=this.$getRow().find(".submit-tableentry-table");b.children(".submit-tableentry-row").remove();for(var c=0;c<a.length;++c){var e=d(b,this);f(e,a[c])}},minRows:e,afterRemarks:!1}}function O(){return z(function(a){$("<tr class='submit-authors-row-default'></tr>").append($("<th>GitHub username</th>").css("width","200px")).append("<th>Type</th>").appendTo(a)},function(a){$("<td class='submit-authors-user-cell'></td>").append($("<img height='28' class='submit-authors-avatar'/>").attr("src",
...
"200px")).append($("<th>Version</th>")).appendTo(a)},function(a){var b=$("<input class='submit-deps-name' size='15'/>"),c=$("<span class='submit-deps-version action'>Choose a version...</span>");b.change(function(){c.text("Choose a version").removeAttr("data-relid")});c.click(function(){A=c;var a=u=Math.random(),d=b.val();m.dialog.dialog("option","title",'Choose a version of "'+b.val()+'"');m.loading.css("display","block");m.error.css("display","none");m.inner.css("display","none");m.dialog.dialog("open");
ajax("submit.deps.getversions",{data:{name:d,owner:"false"},success:function(b){if(a===u)if(0===Object.sizeof(b))m.loading.css("display","none"),m.error.text("There aren't any submitted (and not rejected) plugins on Poggit called "+d).css("display","block");else{m.loading.css("display","none");B=b;m.table.find("tr.submit-deps-version-select-row").remove();m.table.find("tr.submit-deps-version-header").remove();for(var c in b)if(b.hasOwnProperty(c)){var e=$("<span></span>").attr("data-timestamp",b[c].submitTime);
