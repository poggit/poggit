...
ajax("submit.deps.getversions",{data:{name:d,owner:"false"},success:function(a){if(b===w)if(0===Object.sizeof(a))p.loading.css("display","none"),p.error.text("There aren't any submitted (and not rejected) plugins on Poggit called "+d).css("display","block");else{p.loading.css("display","none");C=a;p.table.find("tr.submit-deps-version-select-row").remove();for(var c in a)if(a.hasOwnProperty(c)){var e=$("<span></span>").attr("data-timestamp",a[c].submitTime);timeTextFunc.call(e.get()[0]);p.table.prepend($("<tr></tr>").addClass("submit-deps-version-select-row").append($("<td></td>").append($("<input type='radio'/>").addClass("submit-dep-dialog-version-radio").attr("name",
w).attr("data-value",c).prop("checked",!0)).append($("<label></label>").text(a[c].version))).append($("<td></td>").append($("<input type='checkbox' disabled/>").prop("checked",a[c].preRelease))).append($("<td></td>").append(a[c].stateName)).append($("<td></td>").append(e)))}p.table.prepend("<tr><th>Version</th><th>Pre-release?</th><th>State on Poggit</th><th>Submission date</th></tr>");p.inner.css("display","block")}}})});b.append($("<select class='submit-deps-required'></select>").append("<option value='required'>Required</option>").append("<option value='optional'>Optional</option>").wrap("<td></td>").parent()).append(a.wrap("<td></td>").parent()).append(c.wrap("<td></td>").parent())},
...
a.val()+'"');p.loading.css("display","block");p.error.css("display","none");p.inner.css("display","none");p.dialog.dialog("open");ajax("submit.deps.getversions",{data:{name:d,owner:"true"},success:function(a){if(b===w)if(0===Object.sizeof(a))p.loading.css("display","none"),p.error.text("You do not own any submitted (and not rejected) plugins on Poggit called "+d).css("display","block");else{p.loading.css("display","none");C=a;var c=!0;p.table.find("tr.submit-deps-version-select-row").remove();for(var e in a)if(a.hasOwnProperty(e)){var f=
$("<span></span>").attr("data-timestamp",a[e].submitTime);timeTextFunc.call(f.get()[0]);p.table.append($("<tr></tr>").addClass("submit-deps-version-select-row").append($("<td></td>").append($("<input type='radio'/>").addClass("submit-dep-dialog-version-radio").attr("name",w).attr("data-value",e).prop("checked",c)).append($("<label></label>").text(a[e].version))).append($("<td></td>").append($("<input type='checkbox' disabled/>").prop("checked",a[e].preRelease))).append($("<td></td>").append(a[e].stateName)).append($("<td></td>").append(f)));
c=!1}p.inner.css("display","block")}}})}});$("<div></div>").css("display","flex").append(a).append(c).appendTo(b)},getter:function(){var b=this.$getRow().find("#submit-assoc-parent-version");return"undefined"===typeof b.attr("data-relid")?null:{releaseId:Number(b.attr("data-relid")),name:this.$getRow().find("submit-assoc-parent-name").val(),version:b.attr("data-version")}},setter:function(b){null!==b&&null!==b.version&&($("#submit-assoc-parent-version").text("Change: v"+b.version).attr("data-version",
