...
0>=releaseDetails.myVote&&(l.Approve=h);var t=$("#voteup-dialog").dialog({title:"APPROVE Plugin",autoOpen:!1,position:modalPosition,modal:!0,buttons:l,open:function(){$(".ui-widget-overlay").bind("click",function(){$("#voteup-dialog").dialog("close")})},close:function(){D[0].reset()}});var D=t.find("form").on("submit",function(a){a.preventDefault()});$("#upvote").button().on("click",function(){t.dialog("open")});var u=$("#votedown-dialog").dialog({title:"REJECT Plugin",autoOpen:!1,position:modalPosition,
modal:!0,buttons:{Cancel:function(){u.dialog("close")},Reject:function(){var a=$("#vote-message").val();if(10>a.length)$("#vote-error").text("Please type at least 10 characters...");else{for(var b in["outdate","update","alpha","3.0.0"])if(-1!==a.toLowerCase().indexOf(b)){if(confirm("You may not reject an old plugin just because it doesn't support the API version you use. Moderation action may be triggered upon you if you do so. Are you sure you still want to reject this plugin?"))return;break}y(releaseDetails.releaseId,
-1,a);u.dialog("close");return!0}}},open:function(a,b){$(".ui-widget-overlay").bind("click",function(){$("#votedown-dialog").dialog("close")})},close:function(){E[0].reset()}});var E=u.find("form").on("submit",function(a){a.preventDefault()});$("#downvote").button().on("click",function(){u.dialog("open")})}h=$("#rdesc");l=$("#rchlog");c(h);c(l);if(!1!==sessionData.opts.makeTabs){if(null!==getParameterByName("notabs",null)){$(".release-description").append($("<span class='colored-bullet yellow'></span>").css("cursor",
"pointer").click(function(){window.location=window.location.origin+window.location.pathname}).attr("title","Click to display description in tabs."));return}var v=null;v="html"!==h.attr("data-desc-type")?"The plugin description is not in markdown format.":f("h1 h2 h3 h4 h5 h6".split(" "),h,2,!0,h.attr("id"));null!==v?($("#release-description-bad-reason").html(v),$(".release-description").append($("<span class='colored-bullet red'></span>").css("cursor","pointer").attr("title","Failed to display description in tabs: "+
v))):$(".release-description").append($("<span class='colored-bullet green'></span>").css("cursor","pointer").click(function(){window.location=window.location.origin+window.location.pathname+"?notabs"}).attr("title","Click to display description directly without splitting into tabs."))}var z=[],A=0;l.children("ul").children("li").each(function(){this.hasAttribute("data-disabled")&&z.push(A);A++});l.tabs({disabled:z});$(".delete-release").click(function(){var a={my:"center top",at:"center top+100",
...
open:function(a,b){$(".ui-widget-overlay").bind("click",function(){$("#dialog-confirm").dialog("close")})}})});"#shield-template"===window.location.hash&&(alert("Thank you for submitting your plugin! Please have a look at the shields here and add them to the README on your repo."),window.location.hash="")});
