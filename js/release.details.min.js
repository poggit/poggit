...
"16"))),$("<li>Owner</li>").append($("<ul class='plugin-info release-authors-sub'></ul>").append(a)).prependTo($("#release-authors-main")))});$("#license-dialog").dialog({position:modalPosition,height:.8*window.innerHeight,width:.8*window.innerWidth,autoOpen:!1});releaseDetails.isMine||a();if(sessionData.session.isLoggedIn&&releaseDetails.state===PoggitConsts.ReleaseState.checked){f=function(){var a=$("#votemessage").val();addVote(releaseDetails.releaseId,1,a);p.dialog("close");return!0};g={Cancel:function(){p.dialog("close")},
Accept:f};0>=releaseDetails.myVote&&(g.Accept=f);var p=$("#voteup-dialog").dialog({title:"ACCEPT Plugin",autoOpen:!1,height:300,width:250,position:modalPosition,modal:!0,buttons:g,open:function(){$(".ui-widget-overlay").bind("click",function(){$("#voteup-dialog").dialog("close")})},close:function(){r[0].reset()}});var r=p.find("form").on("submit",function(a){a.preventDefault()});$("#upvote").button().on("click",function(){p.dialog("open")});var q=$("#votedown-dialog").dialog({title:"REJECT Plugin",
autoOpen:!1,height:380,width:250,position:modalPosition,modal:!0,buttons:{Cancel:function(){q.dialog("close")},Reject:function(){var a=$("#votemessage").val();if(10>a.length)$("#vote-error").text("Please type at least 10 characters...");else return addVote(releaseDetails.releaseId,-1,a),q.dialog("close"),!0}},open:function(a,b){$(".ui-widget-overlay").bind("click",function(){$("#votedown-dialog").dialog("close")})},close:function(){t[0].reset()}});var t=q.find("form").on("submit",function(a){a.preventDefault()});
$("#downvote").button().on("click",function(){q.dialog("open")})}});function updateRelease(){var a=$("#setStatus").val();ajax("release.statechange",{data:{relId:releaseDetails.releaseId,state:a},method:"POST",success:function(){location.reload(!0)}})}
