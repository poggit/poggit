...
$(function(){function a(){var a=$("#review-dialog").dialog({title:"Poggit Review",autoOpen:!1,height:380,width:250,position:modalPosition,modal:!0,buttons:{Cancel:function(){a.dialog("close")},"Post Review":function(){var b=$("#reviewcriteria").val(),l=sessionData.session.adminLevel>=PoggitConsts.AdminLevel.MODERATOR?1:2,c=releaseDetails.mainCategory,d=$("#votes").val(),e=$("#reviewmessage").val();addReview(releaseDetails.releaseId,"<?= Session::getInstance()->getName() ?>",b,l,c,d,e);a.dialog("close");
return!0}},open:function(){$(".ui-widget-overlay").bind("click",function(){$("#review-dialog").dialog("close")})},close:function(){b[0].reset()}});var b=a.find("form").on("submit",function(a){a.preventDefault()});$("#addreview").button().on("click",function(){a.dialog("open")})}var b=function(a){var b=a.attr("id"),c=[];a.find("a.anchor").each(function(){var a=$(this).addClass("dynamic-anchor").html("&sect;");a.appendTo(a.parent());var d=this.id.substring(13);c.push(d);var e=b+"-header-"+d;a.parent().attr("data-header-id",
d);a.attr("name",e).attr("id","anchor-"+e).attr("href","#"+e).removeAttr("aria-hidden");dynamicAnchor.call(this)});var l=[];a.find("a[name^='user-content-']").each(function(){var a=$(this);l.push(this.name.substring(13));a.attr("name",b+"-custom-"+a.attr("name"))});a.find("a[href]").each(function(){var a=$(this),d=a.attr("href");switch(/^https?:\/\//i.test(d)?"switchProtocol":d.startsWith("//")?"switchDomain":"/"===d.charAt(0)?"switchPath":"#"!==d.charAt(0)?"switchName":"switchAnchor"){case "switchAnchor":d.startsWith("#user-content-")?
...
[],k=0;g.children("ul").children("li").each(function(){this.hasAttribute("data-disabled")&&h.push(k);k++});g.tabs({disabled:h});f=$("#release-authors");ghApi("users/"+f.attr("data-owner"),{},"GET",function(a){"User"===a.type&&(a=$("<li class='release-authors-entry'></li>").append($("<img/>").attr("src",a.avatar_url).attr("width","16")).append(" @"+a.login).append($("<a></a>").attr("href",a.html_url).attr("target","_blank").append($("<img class='gh-logo'/>").attr("src",getRelativeRootPath()+"res/ghMark.png").attr("width",
"16"))),$("<li>Owner</li>").append($("<ul class='plugin-info release-authors-sub'></ul>").append(a)).prependTo($("#release-authors-main")))});$("#license-dialog").dialog({position:modalPosition,modal:!0,height:.8*window.innerHeight,width:.8*window.innerWidth,autoOpen:!1,open:function(a,b){$(".ui-widget-overlay").bind("click",function(){$("#license-dialog").dialog("close")})}});releaseDetails.isMine||a();if(sessionData.session.isLoggedIn&&releaseDetails.state===PoggitConsts.ReleaseState.checked){f=
function(){var a=$("#votemessage").val();addVote(releaseDetails.releaseId,1,a);p.dialog("close");return!0};g={Cancel:function(){p.dialog("close")},Accept:f};0>=releaseDetails.myVote&&(g.Accept=f);var p=$("#voteup-dialog").dialog({title:"ACCEPT Plugin",autoOpen:!1,height:300,width:250,position:modalPosition,modal:!0,buttons:g,open:function(){$(".ui-widget-overlay").bind("click",function(){$("#voteup-dialog").dialog("close")})},close:function(){r[0].reset()}});var r=p.find("form").on("submit",function(a){a.preventDefault()});
$("#upvote").button().on("click",function(){p.dialog("open")});var q=$("#votedown-dialog").dialog({title:"REJECT Plugin",autoOpen:!1,height:380,width:250,position:modalPosition,modal:!0,buttons:{Cancel:function(){q.dialog("close")},Reject:function(){var a=$("#votemessage").val();if(10>a.length)$("#vote-error").text("Please type at least 10 characters...");else return addVote(releaseDetails.releaseId,-1,a),q.dialog("close"),!0}},open:function(a,b){$(".ui-widget-overlay").bind("click",function(){$("#votedown-dialog").dialog("close")})},
close:function(){t[0].reset()}});var t=q.find("form").on("submit",function(a){a.preventDefault()});$("#downvote").button().on("click",function(){q.dialog("open")})}});function updateRelease(){var a=$("#setStatus").val();ajax("release.statechange",{data:{relId:releaseDetails.releaseId,state:a},method:"POST",success:function(){location.reload(!0)}})}
