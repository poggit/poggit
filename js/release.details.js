...
$(function() {
    const createSectionName = (context, ref) => {
        return `${context}-section-${ref}`;
    };
    const createAnchorName = (context, name) => {
        return `${context}-anchor-${name}`;
    };

    const fixMarkdownLinks = function($content) {
        ...
        $content.find("a.anchor").each(function() { // anchors generated by GitHub
            ...
            $this.attr("id", anchorName).attr("href", "#" + anchorName)
                .removeAttr("aria-hidden");
            dynamicAnchor.call(this);
        });
        const userAnchors = [];
        $content.find("a[name^='user-content-']").each(function() { // anchors created by user, using <a name>
            var $this = $(this);
            const name = this.name.substring("user-content-".length);
            userAnchors.push(name);
            $this.attr("id", createAnchorName(contentId, name));
        });
        $content.find("[id^='user-content-']").each(function() { // anchors created by user, using <a id>
            var $this = $(this);
            const name = this.id.substring("user-content-".length);
            userAnchors.push(name);
            $this.attr("id", `${contentId}-anchor-user-${name}`);
        });
                    ...
                    case LINK_TYPE_ANCHOR:
                        if(link.startsWith("#poggit-")) {
                            ...
                            if(sectionAnchors.indexOf(name) !== -1) {
                                $this.attr(atributeName, "#" + createSectionName(contentId, name));
                            } else if(userAnchors.indexOf(name) !== -1) {
                                $this.attr(atributeName, "#" + createAnchorName(contentId, name));
                            }
                        }
                        break;
                    case LINK_TYPE_NAME:
    ...
    const tabularize = function(tags, $contents, requiredHeaders, isHorizontal, idPrefix) {
        ...
        $contents.children(delimiter).each(function() {
            ...
            var myId = $this.attr("data-base-anchor");
