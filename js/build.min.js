...
function initOrg(b,c){var a=$("<div id='togglewrapper' class='togglewrapper'></div>");a.html("<p>Loading repos...</p>");a.attr("data-name",b);c||a.attr("data-opened","true");var d=toggleFunc(a);ghApi((c?"orgs":"users")+"/"+b+"/repos",{},"GET",function(a){var c=$("<table></table>");c.css("width","100%");for(var e=0;e<a.length;e++){var k=a[e],g="undefined"!==typeof briefEnabledRepos[k.id]?briefEnabledRepos[k.id]:null,l=$("<tr></tr>"),h=$("<td></td>"),f=$("<a></a>");f.text(k.name.substr(0,14)+(14<k.name.length?
"...":""));f.attr("href",getRelativeRootPath()+"ci/"+b+"/"+k.name);f.appendTo(h);f=$("<a><img class='gh-logo' src='"+getRelativeRootPath()+"res/ghMark.png' width='16'/></a>");f.attr("target","_blank");f.attr("href","https://github.com/"+b+"/"+k.name);f.appendTo(h);h.appendTo(l);h=$("<td id=prj-"+k.id+"></td>");null!==g&&g.projectsCount&&h.append(g.projectsCount);h.appendTo(l);h=$("<td></td>");f=$("<div class='toggle toggle-light' id=btn-"+k.id+"></div>");f.css("float","right");f.text(null===g||0===
g.projectsCount?"Enable":"Disable");f.toggles({drag:!0,click:!0,text:{on:"ON",off:"OFF"},animate:250,easing:"swing",checkbox:null,clicker:null,type:"compact"});f.toggles(!(null===g||0===g.projectsCount));f.toggleClass("disabled",!0);f.click(function(b,a){return function(){var c=$("#enableRepoBuilds");c.data("repoId",a.id);var e=null!==b&&0===b.projectsCount,d=e?"Enable":"Disable",g="auto";c.data("target",e?"true":"false");e?(loadToggleDetails(c,a),$("#confirm").attr("disabled",!0)):(g="300px",c.find("#detailLoader").text("Click Confirm to Disable Poggit-CI for "+
a.name),$("#confirm").attr("disabled",!1));c.dialog({title:d+" Poggit-CI",width:g,height:"auto",position:modalPosition});$(".ui-dialog-titlebar button:contains('Close')").prop("title","");c.dialog("open")}}(g,k));f.appendTo(h);h.appendTo(l);l.appendTo(c)}a=$(d);a.empty();c.appendTo(a)});return a}
...
$(function(){var b=$("#inputSearch"),c=$("#inputUser"),a=$("#inputRepo"),d=$("#inputProject"),e=$("#inputBuild"),n=$("#gotoRecent"),t=$("#gotoAdmin"),k=$("#gotoSelf"),g=$("#gotoSearch"),l=$("#gotoUser"),h=$("#gotoRepo"),f=$("#gotoProject"),p=$("#gotoBuild"),m=function(){var b=!c.val().trim(),g=!(c.val().trim()&&a.val().trim()),k=!(c.val().trim()&&a.val().trim()&&d.val().trim()),m=!(c.val().trim()&&a.val().trim()&&d.val().trim()&&e.val().trim());l.hasClass("disabled")!==b&&l.toggleClass("disabled");
h.hasClass("disabled")!==g&&h.toggleClass("disabled");f.hasClass("disabled")!==k&&f.toggleClass("disabled");p.hasClass("disabled")!==m&&p.toggleClass("disabled")};if(""!=window.location.hash){var q=$("a[name="+window.location.hash.substring(1)+"]").parent().offset();"undefined"!==typeof q&&$("html, body").animate({scrollTop:q.top},300)}c.keydown(function(){setTimeout(m,50)});c.change(m);c.keyup(function(a){13==a.keyCode&&l.click()});b.keydown(function(){setTimeout(m,50)});b.change(m);b.keyup(function(a){13==
a.keyCode&&g.click()});a.keydown(function(){setTimeout(m,50)});a.change(m);a.keyup(function(a){13==a.keyCode&&h.click()});d.keydown(function(){setTimeout(m,50)});d.change(m);d.keyup(function(a){13==a.keyCode&&f.click()});e.keydown(function(){setTimeout(m,50)});e.change(m);e.keyup(function(a){13==a.keyCode&&p.click()});k.click(function(){window.location=getRelativeRootPath()+"ci/"+getLoginName()});t.click(function(){window.location=getRelativeRootPath()+"ci"});n.click(function(){window.location=getRelativeRootPath()+
"ci/recent"});g.click(function(){if(""===b.val())$("#searchresults").empty();else{$("#searchresults").text("Loading Search Results...");var a={search:b.val()};ajax("search.ajax",{data:a,method:"POST",success:function(a){var b=$("#searchresults");b.empty();b.html(a.html);b.attr("hidden",!1);$("#inputSearch").val("")},error:function(a,b,c){alert(c)}})}});l.click(function(){$(this).hasClass("disabled")?alert("Please fill in the required fields"):window.location=getRelativeRootPath()+"ci/"+c.val()});
h.click(function(){$(this).hasClass("disabled")?alert("Please fill in the required fields"):window.location=getRelativeRootPath()+"ci/"+c.val()+"/"+a.val()});f.click(function(){$(this).hasClass("disabled")?alert("Please fill in the required fields"):window.location=getRelativeRootPath()+"ci/"+c.val()+"/"+a.val()+"/"+d.val()});p.click(function(){$(this).hasClass("disabled")?alert("Please fill in the required fields"):window.location=getRelativeRootPath()+"ci/"+c.val()+"/"+a.val()+"/"+d.val()+"/"+$("#inputBuildClass").val()+
":"+e.val()});var r=$("#enableRepoBuilds");startToggleOrgs();r.dialog({autoOpen:!1,dialogClass:"no-close",closeOnEscape:!0,close:function(a){a.originalEvent&&$("#detailLoader").empty()},buttons:[{id:"confirm",text:"Confirm",click:function(){$("#confirm").attr("disabled",!0);confirmRepoBuilds($(this),r)}}],modal:!0});16<$("#recentBuilds > div").length&&"on"===getParameterByName("usePages",!1!==sessionData.opts.usePages?"on":"off")&&$("#recentBuilds").paginate({perPage:16,scope:$("div")});12<$("#repolistbuildwrapper > div").length&&
"on"===getParameterByName("usePages",!1!==sessionData.opts.usePages?"on":"off")&&$("#repolistbuildwrapper").paginate({perPage:12,scope:$("div")})});function testWebhook(b,c){ajax("ci.webhookTest",{data:{owner:b,name:c},success:function(a){console.log(a)}})}function promptDownloadResource(b,c){c=prompt("Filename to download with:",c);null!==c&&(window.location=getRelativeRootPath()+"r/"+b+"/"+c)}
