...
ajax("build.scanRepoProjects",{data:{repoId:b.id},success:function(a){if("error/bad_request"===a.status)c.empty(),$("<div><h5>400 - Bad Request</h5><p><br/>"+a.message+"</p></div>").appendTo(c);else if("success"!==a.status)c.empty(),$('<div><p>A internal server error occurred.<br/>Please use this request ID for reference if you need support:\n<code class="code">'+a.requestId+"</code></p></div>").appendTo(c);else{a=a.yaml;var d=(30>a.split(/\r\n|\r|\n/).length?a.split(/\r\n|\r|\n/).length:30)-1,e=
$("<div><h3>"+b.name+"</h3></div>");c.empty();e.appendTo(c);e=$("<div class='cbinput'></div>");var k=$('<input type="checkbox" checked id="manifestEditConfirm">');k.change(function(){document.getElementById("selectManifestFile").disabled=!this.checked;document.getElementById("inputManifestContent").disabled=!this.checked});k.appendTo(e);e.append("Commit default .poggit.yml to the repo");e.appendTo(c);e=$("<p></p>");e.text("After Poggit-CI is enabled for this repo, a manifest file will be created at: ");
