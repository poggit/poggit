...
null!==projectData.preRelease&&b===projectData.preRelease.buildId&&(3<projectData.preRelease.state||3===projectData.preRelease.state&&isLoggedIn()||projectData.writePerm)&&(d=getRelativeRootPath()+"p/"+projectData.preRelease.name+"/"+projectData.preRelease.version);projectData.writePerm&&(1!==a.class||1!==projectData.project.projectType||projectData.project.private||(null===projectData.release&&null===projectData.preRelease?m="Submit":b>(projectData.release?projectData.release.buildId:0)&&b>(projectData.preRelease?
projectData.preRelease.buildId:0)&&(m="Update")),null!==m?(b=$("<button class='btn action'>"+m+"</button>").click(function(){var b=m.toLowerCase();if("submit"===b||"update"===b)$("#wait-spinner").modal(),window.location=getRelativeRootPath()+b+"/"+projectData.path.join("/")+"/"+a.internal}),$("<td class='ci-project-history-action-cell'></td>").append(b).appendTo(c)):(b="A newer build has already been submitted.",projectData.project.private?b="Private plugins cannot be submitted.":2===projectData.project.projectType&&
(b="Virion builds cannot be submitted."),$("<td class='ci-project-history-action-cell' style='text-align: center;'></td>").append($("<span class='ci-project-history-action-cell-text text-danger hover-title' title='"+b+"'>Unavailable</span>")).appendTo(c)));b=getRelativeRootPath()+"babs/"+a.buildId.toString(16);var f=function(){e(a.class,a.internal,!1);return!1};$("<td class='ci-project-history-build-number'></td>").append($("<p></p>").css("margin-bottom","0").append($("<a></a>").text(PoggitConsts.BuildClass[a.class]+
" #"+a.internal).attr("href",b).click(f)).append(g?" <a style='color: red;' href='"+g+"'>[R]</a>":d?" <a style='color: red;' href='"+d+"'>[P]</a>":"")).append($("<p></p>").css("margin-bottom","0").append($("<a></a>").text("(&"+a.buildId.toString(16)+")").attr("href",b).click(f))).appendTo(c);$("<td class='ci-project-history-date'></td>").append($("<span class='time-elapse' data-max-elapse='604800'></span>").attr("data-timestamp",a.date)).appendTo(c);d=PoggitConsts.LintLevel[a.worstLint];$("<td class='ci-project-history-lint'></td>").addClass("lint-style-"+
d.toLowerCase().replace(" ","-")).append($("<p></p>").text(0===a.lintCount?"OK":a.lintCount+" problem"+(1!==a.lintCount?"s":"")).css("margin-bottom","0")).appendTo(c);$("<td class='ci-project-history-commit'></td>").hover(u(!0),u(!1)).append($("<a target='_blank'></a>").attr("href","https://github.com/"+projectData.path[0]+"/"+projectData.path[1]+"/commit/"+a.sha).append($("<code></code>").text(a.sha?a.sha.substring(0,7):"unknown"))).append($("<span></span>").addClass("ci-build-commit-message").attr("data-sha",
