...
d={},g=null,h=null;1===a.class&&1===projectData.project.projectType&&(null!==projectData.release&&(projectData.writePerm&&null===projectData.release?d.submit="Submit":projectData.writePerm&&b>projectData.release.buildId&&b>(projectData.preRelease?projectData.preRelease.buildId:0)?d.submit="Update":b===projectData.release.buildId&&(3<projectData.release.state||3===projectData.release.state&&isLoggedIn())&&(h=getRelativeRootPath()+"p/"+projectData.release.name+"/"+projectData.release.version)),null!==
projectData.preRelease&&(projectData.writePerm&&b>(projectData.release?projectData.release.buildId:0)&&b>projectData.preRelease.buildId?d.update="Update":b===projectData.preRelease.buildId&&(3<projectData.preRelease.state||3===projectData.preRelease.state&&isLoggedIn())&&(g=getRelativeRootPath()+"p/"+projectData.preRelease.name+"/"+projectData.preRelease.version)));b=$("<select><option value='---' style='font-weight: bolder;'>Choose...</option></select>").addClass("ci-project-history-action-select").change(function(){var b=
this.value;if("submit"===b||"update"===b)window.location=getRelativeRootPath()+b+"/"+projectData.path.join("/")+"/"+a.internal;this.value="---"});for(var e in d)b.append($("<option></option>").attr("value",e).text(d[e]));$("<td class='ci-project-history-action-cell'></td>").append(b).appendTo(c);d=getRelativeRootPath()+"babs/"+a.buildId.toString(16);e=function(){f(a.class,a.internal,!1);return!1};$("<td class='ci-project-history-build-number'></td>").append($("<p></p>").css("margin-bottom","0").append($("<a></a>").text(PoggitConsts.BuildClass[a.class]+
" #"+a.internal).attr("href",d).click(e)).append(h?" <a style='color: red;' href='"+h+"'>[R]</a>":g?" <a style='color: red;' href='"+g+"'>[P]</a>":"")).append($("<p></p>").css("margin-bottom","0").append($("<a></a>").text("(&"+a.buildId.toString(16)+")").attr("href",d).click(e))).appendTo(c);$("<td class='ci-project-history-date'></td>").append($("<span class='time-elapse' data-max-elapse='604800'></span>").attr("data-timestamp",a.date)).appendTo(c);g=PoggitConsts.LintLevel[a.worstLint];$("<td class='ci-project-history-lint'></td>").addClass("lint-style-"+
g.toLowerCase().replace(" ","-")).append($("<p></p>").text(0===a.lintCount?"OK":a.lintCount+" problem"+(1!==a.lintCount?"s":"")).css("margin-bottom","0")).appendTo(c);$("<td class='ci-project-history-commit'></td>").append($("<a target='_blank'></a>").attr("href","https://github.com/"+projectData.path[0]+"/"+projectData.path[1]+"/commit/"+a.sha).append($("<code></code>").text(a.sha?a.sha.substring(0,7):"unknown"))).append($("<span></span>").addClass("ci-build-commit-message").attr("data-sha",a.sha).hover(v(!0),
v(!1))).appendTo(c);g=$("<td class='ci-project-history-branch'></td>").appendTo(c);4===a.class?(g.text("#"+a.branch),g.wrapInner($("<a target='_blank'></a>").attr("href","https://github.com/"+projectData.path[0]+"/"+projectData.path[1]+"/pull/"+a.branch))):(g.text(a.branch),g.wrapInner($("<a target='_blank'></a>").attr("href","https://github.com/"+projectData.path[0]+"/"+projectData.path[1]+"/tree/"+a.branch+"/"+a.path)));g=getRelativeRootPath()+"r/"+a.resourceId+"/"+projectData.path[2]+"_"+PoggitConsts.BuildClass[a.class].toLowerCase()+
...
E(a);return a.$row=c}function E(b){ghApi("repositories/"+projectData.project.repoId+"/commits/"+b.sha,{},"GET",function(a){b.commitMessage=a.commit?a.commit.message:"unknown";b.authorName=a.commit?a.commit.author.name:"unknown";b.authorLogin=a.commit?a.author.login:"unknown";F(a.sha,b.commitMessage,b.authorName,b.authorLogin)})}function F(b,a,c,d){$("span.ci-build-commit-message[data-sha='"+b+"']").each(function(){var b=$(this);b.empty();b.text(a.split("\n")[0]);$("<span class='ci-build-commit-details'></span>").text(a.split("\n").slice(1).join("\n")).appendTo(b);
$("<a target='_blank'></a>").attr("href","https://github.com/"+d).attr("title",c).append($("<img width='16'/>").attr("src","https://github.com/"+d+".png?width=20").css("margin","5px")).prependTo(b)})}function v(b){return function(){$(this).find(".ci-build-commit-details").css("display",b?"block":"none")}}var q={},r=null,n=$("#ci-build-inner"),z=$("#ci-build-pane");$("#ci-build-close").click(function(){h(!0)});var C=$("#ci-build-header"),w=!1,x,A=!1,D=null;n.find(".ci-build-commit-message").hover(v(!0),
v(!1));var u=function(){var b;return function(a){if("undefined"!==typeof b)return b;if(a)return null;b=$("<div id='build-pane-dialog'></div>");b.dialog({autoOpen:!1,modal:!0,position:modalPosition,close:function(){A||h(!0)},open:function(a,b){$(".ui-widget-overlay").bind("click",function(){$("#build-pane-dialog").dialog("close")})}});return b}}(),p={showNarrow:function(){var b=u();b.dialog("option","title",C.text());n.appendTo(b);c(b);b.dialog("open")},hideNarrow:function(){var b=u();A=!0;b.dialog("close");
A=!1},showWide:function(){n.appendTo(z);z.css("display","flex")},hideWide:function(){z.css("display","none")}};(function(){function b(a){x=a;w&&(a?(p.hideWide(),p.showNarrow()):(p.hideNarrow(),p.showWide()))}var a=window.matchMedia("(max-width: 1000px)");b(a.matches);a.addListener(function(a){b(a.matches)});$(window).resize(function(){var a=u(!0);null!==a&&c(a)})})();window.addEventListener("popstate",function(){t(!0)});t(!1);var H=document.getElementById("projectPath"),B=/^https:\/\/github\.com\/([a-z\d](?:[a-z\d]|-(?=[a-z\d])){0,38})\/([a-z0-9_.-]+)\/tree\/master\/?(.*)/i.exec(H.href);
ghApi("repos/"+B[1]+"/"+B[2],{},"GET",function(b){H.href="https://github.com/"+b.full_name+"/tree/"+b.default_branch+"/"+B[3]});var I=getParameterByName("branch","special:dev");$("#ci-project-history-branch-select").val(I);y(I,-1,10);$("#ci-project-history-load-more").click(function(){$(this).hasClass("disabled")||($(".ci-project-history-locks").addClass("disabled").prop("disabled",!0),null!==r&&y(r.branch,r.tail,10,r.pr))});$("#ci-project-history-branch-select").change(function(){function b(a,b){return"special:dev"===
b?!0:"special:pr"===b?4===a.class:b===a.branch}var a=$("#ci-project-history-table");a.find(".ci-project-history-content-row").detach();var c=this.value,d=Object.valuesToArray(q);d.sort(function(a,b){return b.date-a.date});for(var g=0,f=-1,e=0;e<d.length;++e)b(d[e],c)&&(a.append(G(d[e].buildId)),++g,-1===f||f>d[e].internal)&&(f=d[e].internal);10>g&&($(".ci-project-history-locks").addClass("disabled").prop("disabled",!0),y(c,f,10-g,"special:pr"===c))})});
