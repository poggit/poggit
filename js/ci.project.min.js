...
e).click(function(){return 210===projectData.project.projectId||confirm("This "+(2===projectData.project.projectType?"virion":"plugin")+" has not been reviewed, and it may contain dangerous code including viruses. Do you still want to download this file?")})).appendTo(c);2===projectData.project.projectType&&$("<td class='ci-project-history-virion-version'></td>").text(a.virionVersion).appendTo(c);D(a);return a.$row=c}function D(b){ghApi("repositories/"+projectData.project.repoId+"/commits/"+b.sha,
{},"GET",function(a){b.commitMessage=a.commit?a.commit.message:"unknown";b.authorName=a.commit?a.commit.author.name:"unknown";b.authorLogin=a.commit?a.author.login:"unknown";E(a.sha,b.commitMessage,b.authorName,b.authorLogin)})}function E(b,a,c,d){$("span.ci-build-commit-message[data-sha='"+b+"']").each(function(){var b=$(this);b.empty();b.text(a.split("\n")[0]);$("<span class='ci-build-commit-details'></span>").text(a.split("\n").slice(1).join("\n")).appendTo(b);$("<a target='_blank'></a>").attr("href",
